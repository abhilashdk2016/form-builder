...
const Designer = () => {
  ...
  useDndMonitor({
    onDragEnd: (event: DragEndEvent) => {
        ...
        const isDroppingOverDesinerDropArea = over.data?.current?.isDesignerDropArea;
        // dropping from sidebar to designer drop area
        if(isDesignerBtnElement && isDroppingOverDesinerDropArea) {
        ...
        // Adding at top or bottom of existing element
        const isDroppingOverDesignerElementTopHalf = over.data?.current?.isTopHalfDesignerElement;
        const isDroppingOverDesignerElementBottomHalf = over.data?.current?.isBottomHalfDesignerElement;
        const droppingSidebarButtonOverDesignerElement  = isDesignerBtnElement && (isDroppingOverDesignerElementTopHalf || isDroppingOverDesignerElementBottomHalf);
        if(droppingSidebarButtonOverDesignerElement) {
            ...
            if(overElementIndex === -1) {
                throw new Error("elementn not found");
            }
            let indexForNewElement = overElementIndex; // Asuming dropping on he top half
        ...
        // Rearrage elements by dragging
        const isDraggingDesignerElement = active.data?.current?.isDesignerElement;
        const rearrangeElementsByDragging = (isDroppingOverDesignerElementTopHalf || isDroppingOverDesignerElementBottomHalf) && isDraggingDesignerElement;
        if(rearrangeElementsByDragging) {
...
const DesignerElementWrapper = ({ element } : { element : FormElementInstance }) => {
    ...
    const DesignerElement = FormElements[element.type].designerComponents;
    return <div ref={draggable.setNodeRef} {...draggable.listeners} {...draggable.attributes} onClick={(e) => {
            e.stopPropagation();
            setSelectedElement(element)}
        }
        className='relative h-[120px] flex flex-col text-foreground hover:cursor-pointer rounded-md ring-1 ring-accent ring-inset' onMouseEnter={() => setMouseIsOver(true)} onMouseLeave={() => setMouseIsOver(false)}>
        <div className={'absolute w-full h-1/2 rounded-t-md'} ref={topHalf.setNodeRef} />
        <div className={'absolute  w-full h-1/2 rounded-b-md bottom-0'} ref={bottomHalf.setNodeRef} />
        {
            mouseIsOver && (
                <>
